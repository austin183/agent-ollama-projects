...
This is a reference implementation based on scripts/archive_files.py
"""
...
def main():
    ...
    script_lines = [
        ...
        f"mkdir -p {format_path_for_shell(str(WORKSPACE_DIR / 'Archive'))}",
        ""
    ]
    ...
    for parent_dir in sorted(group_info.keys()):
        ...
        comment = f"# Group {group_info[parent_dir][0][0]}: {group_info[parent_dir][0][1]} -> Archive"
    ...
    for parent_dir, entries_list in sorted(group_info.items()):
        for folder, filename, rationale in entries_list:
            ...
            archive_path = str(WORKSPACE_DIR / "Archive")
            # Keep the folder path but replace workspace root with Archive root
            if str(folder).startswith(str(WORKSPACE_DIR)):
                dest_path = format_path_for_shell(archive_path + folder[len(str(WORKSPACE_DIR)):])
            else:
                dest_path = format_path_for_shell(archive_path + folder)
