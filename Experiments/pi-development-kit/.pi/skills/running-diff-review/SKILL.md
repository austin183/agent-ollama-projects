...
description: Runs the diff-review subagent with context from project learnings to reduce false positives. Use when invoking the diff-review subagent, before asking it to review code changes.
---

# Running Diff Review

Runs the `diff-review` subagent with project learnings injected into its task, so it can distinguish intentional patterns from regressions.
...
- [ ] Step 4: Invoke the diff-review subagent with an enriched task
...
Use the `subagent` tool with agent `diff-review`, with a task that includes:
...
After the subagent returns its results, discuss the findings with the user before taking any corrective actions.
