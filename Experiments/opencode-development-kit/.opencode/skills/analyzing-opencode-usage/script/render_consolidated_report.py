...
def build_summary_cards(data):
  ...
  <div class="metric-card" title="Total tokens as recorded by opencode: input + output + reasoning. Includes redundant context re-sent each turn in multi-turn sessions."><div class="metric-value">{fmt(raw_total)}</div><div class="metric-label">Raw Tokens</div></div>
  ...
  <div class="metric-card" title="Simulated prefix-caching estimate, not actual provider cache performance. Computed from per-message token deltas: each turn re-sends prior context, but only the delta vs the previous turn is considered truly new."><div class="metric-value">{cache_hit}%</div><div class="metric-label">Cache Hit Rate</div></div>'''
    ...
    cards += (
        ...
        '<strong>Raw Tokens</strong> = all tokens as recorded by opencode, '
    ...
    subtitle = f'{period} &middot; {model_count} models &middot; {agent_count} agent roles &middot; Generated: {meta.get("generated", "")}'
...
def build_agents_section(agents_data, cache_estimate=None):
    """Render horizontal bars for token usage by agent (always visible)."""
    if not agents_data:
    ...
    for d in agents_data:
        name = (d.get('agent') or 'unknown')[:20]
        lookup = lookups['by_agent'].get(name)
        if lookup:
            eff = _effective_total(lookup['uncached_input'], lookup['output'], lookup['reasoning'])
            max_total = max(max_total, eff)
        else:
...
def build_cross_tab(cross_tab_data):
    """Render model x agent breakdown table (collapsible)."""
    if not cross_tab_data:
        return '<p style="color:#8892a4">No cross-tab data in this period.</p>'

    html = '<table><thead><tr><th>Model</th><th>Agent</th><th class="num">Sessions</th><th class="num">Effective Tokens</th></tr></thead><tbody>'
    for d in cross_tab_data:
        model = (d.get('model') or 'unknown')[:30]
        agent = (d.get('agent') or 'unknown')[:20]
        sess = d.get('sessions', 0) or 0
        tot = d.get('total_tokens_raw', 0) or 0
...
def build_code_impact_cards(ts_data):
    ...
    total_adds = sum(r[9] for r in rows)
...
def build_agent_category_section(ts_data):
    ...
    html_parts.append('<h3 class="chart-title">Daily Token Breakdown by Agent Category</h3>')
...
def build_productivity_section(data):
    """Render productivity overview (collapsible)."""
    ...
    notes += '<code>summary_files</code> is only populated for sessions where opencode recorded file changes. '
    notes += 'Most sessions are exploratory, research, or cross-session coordination. '
    notes += 'Build agent productive sessions are estimated from token-ratio approximation '
    notes += '(build tokens / total effective tokens per day) since per-agent daily session counts are not available. '
    ...
    null_agent_tokens = 0
    for r in data.get('agents_detailed', []):
        if r.get('agent') is None:
            null_agent_tokens += r.get('total_tokens', 0) or 0
    if null_agent_tokens > 0:
        notes += f'{null_agent_tokens} tokens from sessions with no agent assigned. '
    notes += '</p>'
...
def build_top_sessions_section(top_sessions_data, cache_estimate=None):
    ...
    html = '<table><thead><tr><th>#</th><th>Agent</th><th>Model</th><th>Title</th><th class="num">Tokens (raw)</th></tr></thead><tbody>'
    for i, d in enumerate(top_sessions_data, 1):
        ...
        model_key = model_raw.replace('_', '-').replace('-', '-')[:30]
        title = str(d.get('title', ''))[:60]
        raw_tot = d.get('tokens', 0) or d.get('total_tokens', 0) or 0

        html += f'<tr><td>{i}</td><td>{agent}</td><td style="font-size:0.75rem">{model_raw}</td><td>{title}</td><td class="num">{fmt(raw_tot)}</td></tr>'
    html += '</tbody></table>'
    return html
...
def _cache_lookups(cache_estimate):
    ...
    lookups = {'by_model': {}, 'by_agent': {}, 'by_day': {}}
    if not cache_estimate:
        return lookups
    ...
    for entry in cache_estimate.get('by_agent', []):
        name = (entry.get('key') or entry.get('agent') or 'unknown')[:20]
        lookups['by_agent'][name] = {
            'uncached_input': entry.get('estimated_uncached_input', 0) or 0,
            'output':         entry.get('total_output', 0) or 0,
            'reasoning':      entry.get('total_reasoning', 0) or 0,
        }
...
<h1>{meta.get('title', '[ProjectName] — LLM Usage & Value Report')}</h1>''')
    ...
    _charts_parts.append(subsection(
        ...
        build_agents_section(data.get('agents', []), cache_estimate)
    ))
    _charts_parts.append(subsection(
        'Daily Token Trend',
    ...
    html_parts.append('''
<footer>
  <p>Data sources: opencode SQLite DB (<code>session</code> + <code>message</code> tables) + git log (all tracked files)</p>
  ...
  <p>Token counts use cache-adjusted (uncached) input by default. Raw values shown for comparison in summary cards and top sessions table.</p>
</footer>
